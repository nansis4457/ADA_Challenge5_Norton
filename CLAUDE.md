...
**작업을 시작하기 전에 `docs/rules.md`를 읽는다.** 협상 불가능한 원칙 11개가 있고,
모든 스펙·구현·리뷰가 여기에 종속된다.

| 문서 | 내용 |
|---|---|
| `docs/rules.md` | 프로젝트 규칙 — 매번 확인 |
...
1. **뷰에 색상·폰트 리터럴을 쓴다** → `DesignSystem` 토큰만 쓴다 (규칙 「색과 글꼴은 토큰으로」)
2. **뷰에 문자열 리터럴을 쓴다** → 카피는 도메인 계층에서 온다 (규칙 「문구는 뷰 밖에」)
3. **습도·풍속으로 단계를 올린다** → 설명·추천만 보정한다 (규칙 「습도·풍속을 단계에 두 번 세지 않는다」)
4. **땀의 양을 단정한다** → `~일 수 있어요` 어투 (규칙 「단정하지 않는다」)
5. **추정치를 확정치처럼 표기한다** → `약 12분`, 고지 문구 (규칙 「추정치는 추정치로」)

## 작업 흐름

기능 구현은 `spec.md` → `spec.md`의 구현 계획 → `tasks.md` 순서를 거친다. 자세한 건 `docs/README.md`.
버그 수정과 오탈자는 예외.
