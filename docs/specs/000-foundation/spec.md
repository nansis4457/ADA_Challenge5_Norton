...
| R4 | 습도·풍속을 바꿔도 단계는 변하지 않는다 | 규칙 「습도·풍속은 등급을 바꾸지 않는다」 회귀 테스트 |
...
| 습도·풍속은 등급을 바꾸지 않는다 | ✅ | `SweatStageEngine.stage(_:)` 시그니처에 습도·풍속을 **받지 않는다.** 구조적으로 불가능하게 만든다 |
...
습도·풍속은 이 경로에 **들어오지 않는다** (규칙 「습도·풍속은 등급을 바꾸지 않는다」). 002에서 설명 문구를 만들 때
...
| 규칙 「습도·풍속은 등급을 바꾸지 않는다」 회귀 | 단위 | 시그니처에 습도·풍속이 없음을 컴파일로 보장 + 문서화 테스트 |
