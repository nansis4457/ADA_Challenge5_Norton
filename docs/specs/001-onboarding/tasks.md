...
- [x] T001 브랜치 `001-onboarding` 생성 (`main` 머지 후)
- [x] T002 spec 상태를 Approved로, plan·tasks 커밋
...
- [x] T012 Figma 컴포넌트를 T010·T011에 맞춰 수정
      `design: Figma 컴포넌트 여백을 코드와 일치시킴` *(코드 변경 없음, 문서만)*

## 도메인

- [x] T020 [P] `Transport` — 도보·지하철·버스·자전거
      `feat(domain): 이동수단 타입 추가`
- [x] T021 [P] `OutdoorDuration` — 20분 이하·20~40분·40분 이상
      `feat(domain): 야외 이동시간 타입 추가`
- [x] T022 `rawValue` 고정 테스트 — 바뀌면 저장 데이터가 깨진다
      `test(domain): 저장 식별자 안정성 검증`

## 영속화

- [x] T030 `SweatPersistence` 패키지 생성 + 링크 (의존: SweatDomain)
      `chore(persistence): SweatPersistence 패키지 추가`
- [x] T031 `UserProfile` — Codable 스칼라 묶음, 기본값 포함
      `feat(persistence): 사용자 프로필 모델 추가`
- [x] T032 `ProfileStore` — 읽기·쓰기·기본값·손상 복구
      `feat(persistence): 키-값 프로필 저장소 추가`
- [x] T033 저장소 테스트 — 왕복, 빈 저장소, **손상 데이터** **(R3, R5)**
      별도 suite name을 써서 실제 UserDefaults를 건드리지 않는다
      `test(persistence): 프로필 저장소 왕복 및 손상 복구 검증`

## 피처 골격

- [x] T040 `SweatFeatures` 패키지 생성 + 링크, MainActor 기본
      `chore(features): SweatFeatures 패키지 추가`
- [x] T041 `lint-tokens.sh` 대상에 SweatFeatures 추가 (규칙 「색과 글꼴은 토큰으로」)
      `chore: 린트 대상에 SweatFeatures 추가`
- [x] T042 `OnboardingCopy` — 스펙 §6의 문구 전부
      `feat(onboarding): 온보딩 문구 정의 추가`
- [x] T043 온보딩 문구에 UX Writing 린트 적용 **(R14)**
      `test(onboarding): 온보딩 문구 금지 표현 검사`

## 화면

- [x] T050 `SensitivityStep` — 3지선다 **(R5, R12)**
      `feat(onboarding): 땀 민감도 선택 화면 추가`
- [x] T051 `TransportStep` — 이동수단 4 + 이동시간 3 **(R5, R12)**
      `feat(onboarding): 이동 패턴 선택 화면 추가`
- [x] T052 `NotificationStep` + 권한 요청 **(R6, R7)**
      `feat(onboarding): 알림 권한 화면 추가`
- [x] T053 `OnboardingFlow` — 단계 진행, 최초/편집 모드 **(R8~R11)**
      `feat(onboarding): 온보딩 흐름과 편집 모드 추가`
- [x] T054 `RootView` — 온보딩 완료 여부로 분기 **(R1, R2)**
      `feat(onboarding): 최초 실행 분기 추가`
- [x] T055 `StagePlaceholderView` — 계산된 단계 표시 **(R4)**. 002가 대체
      편집 진입 버튼을 둬서 R8~R11을 검증한다
      `feat: 등급 확인용 임시 화면 추가`

## 마무리

- [x] T090 접근성 확인 — VoiceOver 선택 상태, `AX5` 잘림 **(R12, R13)**
- [x] T091 흐름 수동 검증 — 최초/편집, 권한 허용·거부
      시뮬레이터 데이터 초기화: `xcrun simctl privacy <UDID> reset all <bundle-id>`
- [x] T092 린트·테스트·Debug/Release 빌드
- [x] T093 규칙 점검 표 최종 확인 (spec.md §2)
- [x] T094 `docs/specs/README.md`의 001 상태 갱신
- [ ] T095 PR 생성
